...
# Mini-expansor de fluxo (igual ao financeiro, simplificado)
def _expandir_fluxo_min(dfl: pd.DataFrame, projeto: str, taxa_anual: float, data_base: date, inflacao_anual: float=0.0, horizonte_meses: int=60) -> pd.DataFrame:
    if dfl.empty:
        return pd.DataFrame(columns=["competencia","valor"]) 
    dfp = dfl[dfl["projeto"]==projeto].copy()
    if dfp.empty:
        return pd.DataFrame(columns=["competencia","valor"]) 
    dfp["valor"] = np.where(dfp["tipo"].str.lower()=="despesa", -pd.to_numeric(dfp["valor"], errors="coerce").fillna(0.0), pd.to_numeric(dfp["valor"], errors="coerce").fillna(0.0))
    linhas = []
    for _, row in dfp.iterrows():
        start = pd.to_datetime(row.get("data_inicio", data_base), errors="coerce").date() if row.get("data_inicio") else data_base
    ...
    if fluxo.empty:
        return fluxo
    fluxo = fluxo.groupby("competencia", as_index=False)["valor"].sum().sort_values("competencia")
    # inflação opcional (preços constantes)
...
def dashboard_principal():
    ...
    # Projetos candidatos
    projetos = sorted(list(set(df_atv["projeto"].dropna().unique().tolist() if not df_atv.empty else []) | set(df_param["projeto"].dropna().unique().tolist() if not df_param.empty else []) | set(df_lanc["projeto"].dropna().unique().tolist() if not df_lanc.empty else [])))
    if not projetos:
        st.info("Cadastre projetos em 'Projetos e Atividades' e parâmetros/lançamentos em 'Financeiro do Projeto'.")
        return
    ...
    for p in projetos:
        ...
        if not df_param.empty and (df_param["projeto"]==p).any():
            ...
            linha.update(l)
        fluxo = _expandir_fluxo_min(df_lanc, p, float(linha["taxa_desconto_anual"]), linha["data_base"], float(linha["indice_inflacao_anual"]), int(linha["horizonte_meses"]))
        vpl = _npv_mensal(fluxo, float(linha["taxa_desconto_anual"]), linha["data_base"]) if not fluxo.empty else 0.0
        rows.append({"projeto": p, "vpl": vpl, "wacc": linha["taxa_desconto_anual"], "moeda": l.get("moeda","BRL") if 'l' in locals() else "BRL"})
    ...
    if df_atv.empty:
        ...
        proximas = df_atv_show.dropna(subset=["fim"]) \
            .query("fim >= @pd.Timestamp('today').normalize() and fim <= @pd.Timestamp('today').normalize() + @pd.Timedelta(days=30)") \
            .sort_values("fim")[["projeto","atividade","responsavel","status","prioridade","fim"]]
        if proximas.empty:
            st.caption("Sem entregas no próximo mês.")
        else:
            proximas["fim"] = proximas["fim"].dt.strftime("%Y-%m-%d")
            st.dataframe(proximas.reset_index(drop=True), use_container_width=True, hide_index=True)
